...
async function initDashboardStats() {
    ...
    try {
        ...
        const animateCount = (element, endValue) => {
            ...
            const stepTime = Math.max(minIncrementTime, Math.abs(Math.floor(duration / range)));
...
function initAdmins() {
    ...
    addAdminForm.addEventListener('submit', async (e) => {
        ...
        if (!existingAdmin.empty) {
            showFeedback(addAdminFeedback, 'Cet administrateur existe déjà.', 'danger');
            return;
        }

        // Ajouter le nouvel admin
        await db.collection('admins').add({ email: email, addedBy: currentUser.email });
        emailInput.value = '';
        showFeedback(addAdminFeedback, 'Administrateur ajouté avec succès.', 'success');
    });
            ...
            if (!isCurrentUser) {
                ...
                deleteButton.onclick = () => {
                    showConfirmation(
                        ...
                        `Êtes-vous sûr de vouloir retirer les droits d'administrateur pour ${admin.email} ? L'utilisateur ne pourra plus se connecter.`,
                        async () => {
                            try {
                                await db.collection('admins').doc(doc.id).delete();
                                showNotification(`L'administrateur ${admin.email} a été retiré.`, 'success');
                            } catch (error) {
                                console.error("Erreur lors du retrait de l'administrateur:", error);
                                showNotification("Une erreur est survenue lors du retrait de l'administrateur.", 'danger');
                            }
                        }
                    );
                };
        ...
        snapshot.forEach(doc => {
            ...
            const tr = `
                <tr class="${rowClass}">
                    ...
                    <td data-label="Actions" class="actions-cell">
                        <button class="btn btn-sm delete-produit-btn" data-id="${doc.id}" data-nom="${produit.nom}"><i data-lucide="trash-2"></i></button>
                    </td>
                </tr>
            `;
...
function attachProduitActionListeners() {
    ...
    if (produitsList.dataset.listenerAttached === 'true') {
        return;
    }
    produitsList.dataset.listenerAttached = 'true';

    // Utiliser la délégation d'événements pour gérer les clics sur les boutons de suppression
    produitsList.addEventListener('click', async (e) => {
        const button = e.target.closest('.delete-produit-btn');
        if (!button) return;

        const id = button.dataset.id;
        const nom = button.dataset.nom || 'ce produit';

        showConfirmation(
            'Supprimer le produit',
            `Êtes-vous sûr de vouloir supprimer "${nom}" ? Cette action est irréversible et supprimera également l'historique des ventes associé.`,
            async () => {
                try {
                    const batch = db.batch();

                    // 1. Supprimer les ventes associées à ce produit
                    const ventesSnapshot = await db.collection('ventes').where('produit_id', '==', id).get();
                    ventesSnapshot.forEach(doc => {
                        batch.delete(doc.ref);
                    });

                    // 2. Supprimer le produit lui-même
                    const produitRef = db.collection('produits').doc(id);
                    batch.delete(produitRef);

                    // Exécuter toutes les opérations
                    await batch.commit();

                    showNotification(`Le produit "${nom}" et son historique de ventes ont été supprimés.`, 'success');
                } catch (error) {
